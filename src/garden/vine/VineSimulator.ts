...
export class VineSimulator {
    ...
    public vinePoints: VinePoint[] = [];
    public thorns: ThornData[] = [];
    public leaves: LeafData[] = [];
    public twigs: TwigData[] = [];
    public attachmentPoints: AttachmentPoint[] = [];
    public totalLength: number = 0;

    constructor(width: number, height: number, _isBackground: boolean) {
        this.width = width;
        this.height = height;
        // isBackground stored in coordinator to decide noise seeds
    }

    public generateVinePath(): void {
        this.vinePoints = [];
        ...
        for (let i = 0; i <= segments; i++) {
            ...
            this.vinePoints.push({
                x, y, thickness: config.vine.thickness, len: 0,
    ...
    public getPointAtT(t: number): VinePoint {
        const idx = Math.floor(Math.max(0, Math.min(1, t)) * (this.vinePoints.length - 1));
        return this.vinePoints[idx];
    }

    public getHelixAngle(len: number, phaseOffset: number, warp: number): number {
        const centerLen = (this.totalLength * 0.5);
