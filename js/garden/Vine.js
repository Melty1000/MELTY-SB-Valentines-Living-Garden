...
import { CrownFlower } from './flowers/CrownFlower.js';
import { noise, lerp } from '../utils/math.js';
import { config } from '../config.js';
import {
  StrandPoint,
  AttachmentPoint
} from './vine/VineTypes.js';
import { VineSimulator } from './vine/VineSimulator.js';
import { VineRenderer } from './vine/VineRenderer.js';
...
export class Vine extends Container {
  ...
  graphics;
  isBackground;
  growth = config.vine.defaultGrowth || 0.03;
  growthTarget = config.vine.defaultGrowth || 0.03;
  timeOffset;
  // Apex Predator Optimization (v6) Pooling
  // allocate once, reuse forever.
  strandPool: StrandPoint[][] = [[], []];
  lastCalculatedStrands: StrandPoint[][] = [[], []];

  constructor(width, height, isBackground = false) {
    super();
  ...
  refresh() {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      this.simulator.attachmentPoints.push({ t, strandIdx, occupied, slotIdx: i % 2 });
      this.simulator.attachmentPoints.push({ t, strandIdx, occupied, slotIdx: (i + 1) % 2 });
    }
  ...
  ensurePoolSize(size) {
    for (let s = 0; s < 2; s++) {
      while (this.strandPool[s].length < size) {
        this.strandPool[s].push({ x, y, t: 0 });
      }
    }
  }
  update(time, _deltaTime, windOffset = 0) {
  ...
  calculateStrands(time, windOffset)[] {
    ...
    for (let s = 0; s < 2; s++) {
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        // THE KEY =0 uses +sin, s=1 uses -sin (OPPOSITE direction)
  ...
  getCrownPosition(time, windOffset = 0): { x; y: number } {
  ...
  getStrandPosition(t, strandIdx, time, windOffset = 0) | null {
    const pts = this.simulator.vinePoints;
  ...
  getAvailableAttachmentPoint(externalOccupied = [], preferredSlotType = 0) | null {
    ...
    const spacingReduction = count > compressionThreshold ? (count - compressionThreshold) * 0.0001 ;
    ...
    const activeCandidates = typeMatchingCandidates.length > 0 ? typeMatchingCandidates ;
