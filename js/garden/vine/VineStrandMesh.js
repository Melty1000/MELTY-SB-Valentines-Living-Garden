...
import { StrandPoint } from './VineTypes.js';
...
export class VineStrandMesh {
  ...
  updatePositions(points, growth) {
        ...
        for (let i = 0; i < points.length; i++) {
            const pt = points[i];
            const baseIdx = i * 4;  // 4 floats per point, leftY, rightX, rightY
  ...
  updateMeshGeometry(mesh, widthScale) {
